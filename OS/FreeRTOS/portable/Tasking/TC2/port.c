...
#define portENABLE_CPU_INTERRUPT 			( 1U << 12U )  // l-m: not used. ( 1U << 10U ) for tc297
/*-----------------------------------------------------------*/
...
static void prvSystemTickHandler( int );
#endif
...
#define ulCompareMatchValue	( configPERIPHERAL_CLOCK_HZ / configTICK_RATE_HZ )
...
//	uint32_t *pulLowerCSA = NULL;
	...
	//vTrapInstallHandlers();  // LM: used in cint_tc29x.c. Within iLLD, Trap handlers are installed in the Cpu/Trap/IfxCpu_Trap.c with hooks for extension.

	/* Install the Syscall Handler for yield calls. */
//	if( 0 == _install_trap_handler( portSYSCALL_TRAP, prvTrapYield ) )  // LM: prvTrapYield() is the handler of syscall(0) trap,
//	{																	//     be called in syscall trap's hook defined in Ifx_Cfg_trap.h
//		/* Failed to install the yield handler, force an assert. */		//     instead of being installed directly.
...
//	}
	...
	switches from an ISR. Table. Interrupt Priority 1 is the first and
	lowest-priority entry in the Interrupt Vector Table.It is generally
	reserved for ISRs which perform task management. */
#ifdef IFX_USE_SW_MANAGED_INT
	IfxCpu_Irq_installInterruptHandler( prvInterruptYield, configKERNEL_YIELD_PRIORITY );
#endif
	IfxSrc_init( &SRC_GPSR00, IfxSrc_Tos_cpu0, configKERNEL_YIELD_PRIORITY );
	IfxSrc_enable( &SRC_GPSR00 );  // LM: these 3 lines enable the GPSR00 to trigger a software interrupt.
	...
	_mtcr( CPU_SYSCON, portINITIAL_SYSCON );
	_isync();
...
void prvSetupTimerInterrupt( void )
{
	...
	stmCompareConfig.ticks = ulCompareMatchValue;
	stmCompareConfig.triggerPriority = configKERNEL_INTERRUPT_PRIORITY;
	stmCompareConfig.typeOfService = IfxSrc_Tos_cpu0;

	IfxStm_initCompare( &MODULE_STM0, &stmCompareConfig );

#ifdef IFX_USE_SW_MANAGED_INT
	IfxCpu_Irq_installInterruptHandler( prvSystemTickHandler, configKERNEL_INTERRUPT_PRIORITY );
#endif

	/* Suspend STM0 when CPU is halted by debugger */
...
#else
IFX_INTERRUPT( prvSystemTickHandler, 0, configKERNEL_INTERRUPT_PRIORITY )
#endif
{
    ...
    uint32_t ulCurrentTime = MODULE_STM0.TIM0.U;
    uint32_t ulCompare = MODULE_STM0.CMP[0].U;
...
//	IfxStm_increaseCompare( &MODULE_STM0, IfxStm_Comparator_0, ulCompareMatchValue );
    /* Check if compare is more than one tick behind current time */
    if( ( ulCurrentTime - ulCompare ) > ulCompareMatchValue )
    {
        /* We've drifted - resync compare to current time */
        IfxStm_updateCompare( &MODULE_STM0, IfxStm_Comparator_0,
                              ulCurrentTime + ulCompareMatchValue );
    }
    else
    {
        /* Normal operation - just increment */
        IfxStm_increaseCompare( &MODULE_STM0, IfxStm_Comparator_0,
                                ulCompareMatchValue );
    }
	...
	if( lYieldRequired != pdFALSE )
	{
        ...
        *pxCurrentTCB = _mfcr(CPU_PCXI);
		...
		SRC_GPSR00.B.SRR = 0;  // l-m: Service Request Flag(rh). this operation has no effect according to user manual, and can be commented(based on experiment). but port.c codes like this.
		_isync();
	}
}
/*-----------------------------------------------------------*/
...
#else
IFX_INTERRUPT( prvInterruptYield, 0, configKERNEL_YIELD_PRIORITY )
#endif
{
	uint32_t *pxUpperCSA = NULL;
	uint32_t xUpperCSA = 0UL;
